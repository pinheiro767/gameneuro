...
const a11yBtn=$("#a11yBtn"),librasBtn=$("#librasBtn"),accessibilityModal=$("#accessibilityModal"),librasModal=$("#librasModal");
const fontDownBtn=$("#fontDownBtn"),fontUpBtn=$("#fontUpBtn"),contrastBtn=$("#contrastBtn"),motionBtn=$("#motionBtn"),readBtn=$("#readBtn"),resetAccessBtn=$("#resetAccessBtn");
const closeAccessibility=$("#closeAccessibility"),closeLibras=$("#closeLibras"),librasVideo=$("#librasVideo"),librasFallback=$("#librasFallback"),speakInstructionsBtn=$("#speakInstructionsBtn"),gameStatus=$("#gameStatus");
...
function resize(){const box=canvas.parentElement.getBoundingClientRect();const dpr=Math.min(devicePixelRatio,2);canvas.width=box.width*dpr;canvas.height=box.height*dpr;ctx.setTransform(dpr,0,0,dpr,0,0);radius=Math.max(22,Math.min(38,(box.width-12)/(cols*2+1)));draw()}
...
function update(){levelText.textContent=level;scoreText.textContent=score;livesText.textContent="♥ ".repeat(lives).trim();healthText.textContent=Math.max(0,health)+"%";healthBar.style.width=Math.max(0,health)+"%";bombCount.textContent=bombs;currentImg.src=TYPES[current].img;currentImg.alt=TYPES[current].name;nextImg.src=TYPES[next].img;bombBtn.classList.toggle("armed",armed);gameStatus.textContent=`Fase ${level}. ${score} pontos. ${lives} vidas. A Demência está com ${Math.max(0,health)} por cento de resistência. Próxima estrutura: ${TYPES[current].name}.`}
...
function checkState(){if(health<=0){playing=false;shot=null;playSound("win");openResult(true)}else if(grid[rows-1].some(v=>v!==null)){playing=false;shot=null;openResult(false)}}
function resetResultStyle(){modalImg.classList.remove("event-gif");modal.querySelector(".modal-card").classList.remove("game-over")}
function openIntro(){resetResultStyle();modalImg.src="assets/gif/abertura.gif";modalImg.classList.add("event-gif");modalImg.alt="Animação de abertura da missão neural";modalTitle.textContent="Prepare-se para a missão!";modalBody.innerHTML="<p>Observe a próxima estrutura, mire com calma e forme grupos de três ou mais.</p>";modalAction.textContent="INICIAR JOGO";modalAction.onclick=()=>{modal.close();newGame()};modal.showModal()}
function openResult(win){resetResultStyle();if(win){modalImg.src="assets/gif/fase-concluida.gif";modalImg.classList.add("event-gif");modalImg.alt="Animação de fase concluída";modalTitle.textContent=`FASE ${level} CONCLUÍDA!`;modalBody.innerHTML=`<p>Você protegeu a rede neural e alcançou <strong>${score} pontos</strong>.</p>`;modalAction.textContent="PRÓXIMA FASE"}else{modalImg.src="assets/img/5.png";modalImg.alt="Monstro da Demência";modal.querySelector(".modal-card").classList.add("game-over");modalTitle.textContent="GAME OVER";modalBody.innerHTML=`<p>A Demência alcançou o limite do tabuleiro.</p><p><strong>Pontuação final: ${score}</strong></p>`;modalAction.textContent="JOGAR NOVAMENTE"}modalAction.onclick=()=>{modal.close();resetResultStyle();if(win){level++;health=100;bombs=Math.min(3,bombs+1);lives=3;makeGrid();playing=true;update();requestAnimationFrame(()=>{resize();draw()})}else newGame()};modal.showModal()}
function how(){resetResultStyle();modalImg.src="assets/img/2.png";modalImg.alt="Cérebro vermelho";modalTitle.textContent="Como jogar";modalBody.innerHTML="<ol><li>Toque no tabuleiro para lançar.</li><li>Una 3 ou mais estruturas iguais.</li><li>Cada combinação enfraquece a Demência.</li><li>Ative a bomba para limpar uma área.</li></ol><p>Este é um jogo educativo; não diagnostica nem previne doenças.</p>";modalAction.textContent="ENTENDI";modalAction.onclick=()=>modal.close();modal.showModal()}
playBtn.onclick=()=>{playSound("click");openIntro()};howBtn.onclick=how;closeModal.onclick=()=>{modal.close();resetResultStyle()};homeBtn.onclick=()=>{playing=false;shot=null;show("start")};soundBtn.onclick=()=>{sound=!sound;soundBtn.textContent=sound?"🔊":"🔇";soundBtn.ariaLabel=sound?"Desligar som":"Ligar som";if(sound)playSound("click")};bombBtn.onclick=()=>{if(bombs&&playing){armed=!armed;playSound("click");update()}};
...
const access={font:Number(localStorage.getItem("nb-font")||16),contrast:localStorage.getItem("nb-contrast")==="1",motion:localStorage.getItem("nb-motion")==="1"};
function applyAccess(){document.documentElement.style.setProperty("--user-font-size",access.font+"px");document.documentElement.classList.toggle("high-contrast",access.contrast);document.documentElement.classList.toggle("reduce-motion",access.motion);contrastBtn.setAttribute("aria-pressed",String(access.contrast));motionBtn.setAttribute("aria-pressed",String(access.motion));localStorage.setItem("nb-font",access.font);localStorage.setItem("nb-contrast",access.contrast?"1":"0");localStorage.setItem("nb-motion",access.motion?"1":"0");setTimeout(resize,50)}
function speak(text){if(!("speechSynthesis"in window)){alert("A leitura em voz alta não está disponível neste navegador.");return}speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(text);u.lang="pt-BR";u.rate=.9;speechSynthesis.speak(u)}
a11yBtn.onclick=()=>accessibilityModal.showModal();closeAccessibility.onclick=()=>accessibilityModal.close();
fontUpBtn.onclick=()=>{access.font=Math.min(22,access.font+2);applyAccess()};fontDownBtn.onclick=()=>{access.font=Math.max(14,access.font-2);applyAccess()};
contrastBtn.onclick=()=>{access.contrast=!access.contrast;applyAccess()};motionBtn.onclick=()=>{access.motion=!access.motion;applyAccess()};
readBtn.onclick=()=>speak(screens.game.classList.contains("active")?gameStatus.textContent:"Neuro Bolha. Proteja suas conexões. Agrupe três estruturas iguais e enfrente o monstro da Demência.");
resetAccessBtn.onclick=()=>{access.font=16;access.contrast=false;access.motion=false;applyAccess()};
librasBtn.onclick=()=>librasModal.showModal();closeLibras.onclick=()=>{librasVideo.pause();librasModal.close()};
librasVideo.addEventListener("loadeddata",()=>librasFallback.hidden=true);librasVideo.addEventListener("error",()=>{librasVideo.hidden=true;librasFallback.hidden=false});
speakInstructionsBtn.onclick=()=>speak("Toque no tabuleiro para lançar. Una três ou mais estruturas iguais. Cada combinação enfraquece a Demência. Use a bomba neural para limpar uma área.");
applyAccess();
